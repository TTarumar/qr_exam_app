  ...
  void foundCode(Barcode barcode, MobileScannerArguments arguments) {
    ...
    Future.delayed(Duration(seconds: 1),(){
      if(code.toString()=="Sistem Analizi Ve Tasarımı"){
        Navigator.push(context, MaterialPageRoute(builder: (context)=>ExamPage(courses: "Sistem Analizi Ve Tasarımı")));
      }
      else{
        if(code.toString()=="Bilgisayar Ve Programlamaya Giriş"){
          Navigator.push(context, MaterialPageRoute(builder: (context)=>ExamPage(courses:"Bilgisayar Ve Programlamaya Giriş")));

        } if(code.toString().contains("Algoritmalar ve Programlama")){
          Navigator.push(context, MaterialPageRoute(builder: (context)=>ExamPage(courses:"Algoritmalar ve Programlama")));

        } if(code.toString()=="Temel Bilgi Teknolojileri"){
          Navigator.push(context, MaterialPageRoute(builder: (context)=>ExamPage(courses:"Temel Bilgi Teknolojileri")));

        } if(code.toString()=="Ağ Yönetimi Ve Bilgi Güvenliği"){
          Navigator.push(context, MaterialPageRoute(builder: (context)=>ExamPage(courses:"Ağ Yönetimi Ve Bilgi Güvenliği")));

        } if(code.toString()=="Veritabanı Programlama"){
          Navigator.push(context, MaterialPageRoute(builder: (context)=>ExamPage(courses:"Veritabanı Programlama")));
        }
